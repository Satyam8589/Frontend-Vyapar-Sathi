...
const TAB_STYLE = (active) => ({
  ...
  border: 'none', background: 'none', borderBottom: active ? '2.5px solid #4f46e5' : '2.5px solid transparent',
  color: active ? '#4f46e5' : '#6b7280', transition: 'all 0.2s'
});
...
const StaffPageContent = () => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8fafc', padding: '32px 24px', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ maxWidth: 960, margin: '0 auto' }}>
        {/* Page Header */}
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ margin: 0, fontSize: 26, fontWeight: 800, color: '#111' }}>🛡️ Staff & Permissions</h1>
          <p style={{ margin: '6px 0 0', fontSize: 14, color: '#6b7280' }}>
            Manage your store team — invite employees, create roles, and control access.
          </p>
        </div>
        ...
        <RoleEditorModal isOpen={roleEditorOpen} onClose={() => setRoleEditorOpen(false)} />
      </div>
    </div>
  );
};
