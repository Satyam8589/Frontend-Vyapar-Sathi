...
import {
  X, Smartphone, CheckCircle, AlertCircle, Loader2, Eye, EyeOff,
} from "lucide-react";
 ...
 */
const StoreSettingsModal = ({ isOpen, onClose, store, onStoreUpdated }) => {
  const [upiId, setUpiId]     = useState(store?.settings?.upiId   || "");
  const [upiName, setUpiName] = useState(store?.settings?.upiName || store?.name || "");
  ...
  return (
    ...
    >
      <div
        ...
        style={{ animation: "settingsIn 0.25s cubic-bezier(0.34,1.56,0.64,1)" }}
      >
        {/* Header */}
        ...
        <div className="px-6 py-5 space-y-5 overflow-y-auto max-h-[70vh]">
          ...
          >
            ...
            <div className="text-xs text-indigo-700 leading-relaxed">
              ...
              &nbsp;<strong>UPI</strong>, a QR code is automatically generated with the
              &nbsp;<strong>exact bill amount</strong> — customers scan and pay in one tap.
            </div>
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              value={upiId}
              onChange={(e) => { setUpiId(e.target.value); setShowPreview(false); }}
            ...
            {upiId && !isValidUpiId(upiId) && (
              <p className="text-xs text-red-500 mt-1 flex items-center gap-1">
                <AlertCircle size={12} /> Format: yourname@bankname  (e.g. 9876543210@paytm)
              </p>
            )}
            {upiId && isValidUpiId(upiId) && (
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">
              Payee Name <span className="text-gray-400 font-normal">(shown in customer's UPI app)</span>
            </label>
            <input
              type="text"
              value={upiName}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50">
          ...
          <button
            onClick={handleSave}
            disabled={saving || !hasChanged || (upiId.trim() && !isValidUpiId(upiId))}
            ...
            style={{
              ...
              boxShadow: saving || !hasChanged ? "none" : "0 4px 14px rgba(79,70,229,0.35)",
            }}
          >
            {saving ? (
