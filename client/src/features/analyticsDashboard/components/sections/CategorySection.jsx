  ...
  return (
    <section className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between lg:gap-4">
        <div>
          <p className="text-[9px] font-black uppercase tracking-[0.35em] text-emerald-600 sm:text-[10px]">Categories</p>
          <h3 className="mt-1 text-lg font-black tracking-tight text-slate-900 sm:mt-2 sm:text-2xl">Sales by category</h3>
        </div>
      </div>
      ...
      <div className="mt-5 overflow-x-auto rounded-[1.75rem] border border-slate-200">
        <table className="w-full border-collapse text-left min-w-max">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.category} className="border-t border-slate-100 hover:bg-slate-50">
                <td className="px-3 py-3 text-xs font-black text-slate-900 sm:px-4 sm:py-4 sm:text-sm">{row.category}</td>
                <td className="px-3 py-3 text-xs font-semibold text-slate-600 sm:px-4 sm:py-4 sm:text-sm">₹{Number(row.revenue || 0).toLocaleString('en-IN')}</td>
                <td className="px-3 py-3 text-xs font-semibold text-slate-600 sm:px-4 sm:py-4 sm:text-sm">{Number(row.unitsSold || 0).toLocaleString('en-IN')}</td>
              </tr>
            ))}
          </tbody>
        </table>
