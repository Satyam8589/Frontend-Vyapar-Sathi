  ...
  return (
    <section className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between lg:gap-4">
        <div>
          <p className="text-[9px] font-black uppercase tracking-[0.35em] text-violet-600 sm:text-[10px]">Product drill-down</p>
          <h3 className="mt-1 text-lg font-black tracking-tight text-slate-900 sm:mt-2 sm:text-2xl">{productOverview?.product?.name || selectedProduct.productName}</h3>
          <p className="mt-1 text-xs font-semibold text-slate-500 sm:mt-2 sm:text-sm">
            {productOverview?.product?.category || selectedProduct.category || 'General'} • Stock {Number(productOverview?.product?.currentStock || 0).toLocaleString('en-IN')}
          </p>
        </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-4">
        <div className="rounded-2xl bg-slate-50 px-3 py-3 sm:px-4 sm:py-4">
          <p className="text-[9px] font-black uppercase tracking-wider text-slate-500 sm:text-[10px]">Revenue</p>
        ...
        <div className="rounded-2xl bg-slate-50 px-3 py-3 sm:px-4 sm:py-4">
          <p className="text-[9px] font-black uppercase tracking-wider text-slate-500 sm:text-[10px]">Units Sold</p>
          <p className="mt-1 text-lg font-black text-slate-900 sm:mt-2 sm:text-2xl">{Number(productOverview?.summary?.totalUnits || 0).toLocaleString('en-IN')}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 px-3 py-3 sm:px-4 sm:py-4">
          <p className="text-[9px] font-black uppercase tracking-wider text-slate-500 sm:text-[10px]">Orders</p>
          <p className="mt-1 text-lg font-black text-slate-900 sm:mt-2 sm:text-2xl">{Number(productOverview?.summary?.totalOrders || 0).toLocaleString('en-IN')}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 px-3 py-3 sm:px-4 sm:py-4">
          <p className="text-[9px] font-black uppercase tracking-wider text-slate-500 sm:text-[10px]">Stock Cover</p>
          <p className="mt-1 text-lg font-black text-slate-900 sm:mt-2 sm:text-2xl">
