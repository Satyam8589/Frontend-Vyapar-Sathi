...
const SlowMovingSection = ({ slowMoving, loading, error }) => {
  ...
  if (loading) {
    return (
      <EmptyState
        title="Loading slow-moving products"
        description="Inactive and slow stock are being evaluated."
      />
    );
  }
  ...
  if (error) {
    return (
      <EmptyState
        title="Slow-moving products unavailable"
        description={error}
      />
    );
  }
  ...
  if (!rows.length) {
    return (
      <EmptyState
        ...
        description="Nothing is currently below the inactivity threshold."
      />
    );
  }
            ...
            {rows.map((row) => (
              ...
              >
                ...
                <td className="px-3 py-3 text-xs font-semibold text-slate-600 sm:px-4 sm:py-4 sm:text-sm">
                  {Number(row.daysSinceLastSale || 0)}
                </td>
                <td className="px-3 py-3 text-xs sm:px-4 sm:py-4 sm:text-sm">
                  <span
                    className={`inline-flex rounded-full border px-2 py-1 text-[8px] font-black uppercase tracking-[0.2em] sm:px-3 sm:text-[10px] ${URGENCY_STYLE[row.urgency] || URGENCY_STYLE.low}`}
                  >
                    {row.urgency}
                  </span>
                </td>
